<script lang="ts">
	...
	import { numberStates, mode } from "../../stores";
...
<div class:preventChange class="keyboard">
	...
	<div class="row">
		<Key number="ENTER" on:keystroke={() => !disabled && dispatch("submitWord")} />
		{#each keys[2] as number}
			<Key
				{number}
				on:keystroke={(e) => appendValue(e.detail)}
				state={$numberStates[number]}
			/>
		{/each}
		<Key number="" on:keystroke={backspaceValue}>
